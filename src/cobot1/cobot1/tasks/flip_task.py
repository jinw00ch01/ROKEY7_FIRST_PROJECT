...
class FlipTask:
    ...
    def flip_item_with_spatula(self) -> bool:
        ...
        # 하강 시퀀스
        io.move_line_safe(p['down_1'], vel=150, acc=60)
        io.move_line_safe(p['down_2'], vel=60, acc=60)
        io.move_line_safe(p['down_3'], vel=60, acc=60)

        # 그릴 위 이동
        io.move_line_safe(p['grill_1'], vel=60, acc=60)
        ...
        # 스윕 (쓸어내기)
        io.move_line_safe(p['sweep_1'], vel=60, acc=60)
        io.move_line_safe(p['sweep_2'], vel=60, acc=60)
        io.move_line_safe(p['sweep_3'], vel=60, acc=60)
        io.move_line_safe(p['sweep_4'], vel=60, acc=60)

        release_force()
        release_compliance_ctrl()

        # 들어올리기 → 접시로 이동
        io.move_line_safe(p['lift_up'], vel=60, acc=60)
        io.move_line_safe(p['plate_up'], vel=60, acc=60)
        movel(p['plate_down'], time=0.4)

        # 복귀
        io.move_line_safe(p['back_home'], vel=60, acc=60)
