...
JOINT_POSES = {
    ...
    # source_test 전용
    'source_bottle1': [-15, 41, 59, 86, -70, 9],
    'source_bottle2': [-15, 46, 68, 81, -71, 24],

    # powder_test 전용
    'powder_bottle_lift': [-19, 21, 85, -96, 64, 18],
    'powder_bottle_pick': [-19, 29, 96, -104, 68, 38],
}
...
def get_poses() -> dict:
    ...
    return {
        ...
        'dough': {
            'pick_start_1':  posx([230, 202, -5, 36, -179, 36]),
            'pick_start_2':  posx([230, 202, 150, 36, -179, 36]),
            'dough_start_1': posx([461, 241, 150, 36, -178, 36]),
            'dough_start_2': posx([461, 241, 6, 36, -178, 36]),
            'dough_end_1':   posx([323, -10, 35, 17, -177, 17]),
            'dough_end_2':   posx([323, -10, 150, 17, -177, 17]),
            'pick_end':      posx([230, 202, 0, 36, -179, 36]),
        },
        ...
        'press': {
            ...
            'press_down':     posx([328, -107, 125, 174, 179, 175]),
        ...
        'spatula': {
            ...
            'sweep_4':       posx([448, 10, 30, 87, -111, 179]),
            'lift_up':       posx([448, 10, 297, 87, -111, 179]),

            'plate_up':      posx([730, -128, 252, 119, -109, -175]),
            'plate_down':    posx([750, -108, 222, 119, -109, 51]), #rz:111

            'back_home':     posx([342, -141, 355, 94, -122, 85]),
            'anchor_back_0': posx([374, -348, 311, 61, -173, -176]),
            'anchor_back_1': posx([374, -348, 130, 61, -173, -176]), # z = 136
        },
        ...
        'source': {
            ...
            'plate1':  posx([793, -186, 280, 139, -93, -89]),
            'plate2':  posx([793, -186, 280, 139, -93, 90]),
        },

        # ===== 가루 (powder_test) =====
        'powder': {
            'bottle_lift':  posx([511, -509, 397, 96, -91, 90]),
            'bottle_pick':  posx([511, -509, 265, 96, -91, 90]),
