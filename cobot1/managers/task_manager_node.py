...
class TaskManagerNode(Node):
    def __init__(self):
        ...
        # 파라미터
        self.declare_parameter('source', 'none')
        ...
        self.source_task = SourceTask(self, io=self.io)
        self.powder_task = PowderTask(self, io=self.io)
        self.plate_setting_task = PlateSettingTask(self, io=self.io)
    ...
    def run_workflow(self) -> bool:
        ...
        self.get_logger().info(f'Workflow start | source={source}, powder={powder}')
        ...
        steps = [
            ...
            ('3/6', '접시 배치',           self._step_place_plate),  # 임시 비활성화
            ('4/6', '뒤집개로 뒤집기',     self._step_flip_item),
            ('5/6', '소스 뿌리기',         lambda: self._step_source(source)),
    ...
    def _step_place_plate(self) -> bool:
        self.plate_setting_task.pick_and_place_plate()
        return True
    ...
    def _step_source(self, source: str) -> bool:
        if source == 'none':
            self.get_logger().info('Source skipped.')
            return True
        self.tool_manager.pick_tool('source_bottle')
        self.source_task.dispense_source()
        self.tool_manager.return_tool('source_bottle')
        return True
