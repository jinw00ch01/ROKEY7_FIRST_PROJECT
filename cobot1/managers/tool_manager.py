...
class ToolManager:
    ...
    def pick_tool(self, tool_name: str) -> bool:
        ...
        dispatch = {
            ...
            'source_bottle': self._pick_source_bottle,
    ...
    def return_tool(self, tool_name: str) -> bool:
        ...
        dispatch = {
            ...
            'source_bottle': self._return_source_bottle,
    ...
    def _return_tongs(self):
        ...
        io.move_line_safe(p['pick_end'], vel=200, acc=DEFAULT_ACC)
        io.gripper_open('90mm')
        ...
        for attempt in range(max_attempts):
            ...
            while check_motion() != 0:
                ...
                if cur_fz - base_fz > FORCE_THRESHOLD:
                    force_detected = True
                    self._logger.info(
                        f'[Spatula return] Force spike: {cur_fz:.1f}N '
                        f'(base: {base_fz:.1f}N, diff: {cur_fz - base_fz:.1f}N)')
                    break
                wait(0.1)
            ...
            if not force_detected:
                ...
                io.move_joint_safe(JOINT_POSES['ready'], vel=60, acc=60)
                return
    ...
    def _pick_source_bottle(self):
        io = self._io
        p = self._get_poses()['source']
        io.gripper_open('90mm')
        io.move_joint_safe(JOINT_POSES['ready'])
        io.move_joint_safe(JOINT_POSES['source_bottle1'])
        io.move_line_safe(p['bottle2'])
        io.gripper_close('40mm')
        io.move_line_safe(p['bottle1'])

    def _return_source_bottle(self):
        io = self._io
        p = self._get_poses()['source']
